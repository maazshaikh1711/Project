...
def login_tab():
        ...
        isLoggedIn = True if result else False
        print("result", result)
       

        if st.sidebar.button("Login") or isLoggedIn:
    ...
    if st.sidebar.button("Signup"):
        if username1!='' and password1!='':
            if login_user(username1, password1):
                st.warning("Please use any other username. Username with this name exists!!")
            ...
            st.warning("Please fill the credentials")
...
def main():
    ...
    if choice =="Home":
        ...
        st.write("Kindly login from the dropdown menu on the left side of the screen.")
